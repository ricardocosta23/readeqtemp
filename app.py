...
def submit_readequacao():
    try:
        ...
        # Get all deleted sections
        deleted_sections = []
        for section in ['1A', '1B', '1C', '2A', '2B', '2C', '3A', '3B', '3C', '4A', '4B', '4C']:
            delete_button = request.form.get(f'deleted_{section}')
            if delete_button == 'true':
                deleted_sections.append(section)

        # Map sections to Monday.com field IDs
        section_to_field = {
            '1A': 'texto16__1',
            '2A': 'dup__of_op__o_1c0__1',
            '3A': 'dup__of_op__o_2c__1',
            '4A': 'dup__of_op__o_3c9__1',
            '1B': 'dup__of_op__o_1a__1',
            '1C': 'text0__1',
            '2B': 'dup__of_op__o_1c5__1',
            '2C': 'dup__of_op__o_1c__1',
            '3B': 'dup__of_op__o_3a__1',
            '3C': 'dup__of_op__o_3b__1',
            '4B': 'dup__of_op__o_3c4__1',
            '4C': 'dup__of_op__o_3c__1'
        }

        # Process fields based on deletion status and new values
        for section, monday_field in section_to_field.items():
            form_field = f'novaOpcao{section}'
            field_value = request.form.get(form_field)
            delete_button = request.form.get(f'deleted_{section}')

            if delete_button == 'true':
                # Set the specific column to an empty string for Monday.com text fields
                column_values[monday_field] = {"text": ""}
            elif field_value:
                column_values[monday_field] = {"text": field_value}
